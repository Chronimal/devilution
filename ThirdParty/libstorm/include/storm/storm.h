...
#define GAMEINFO_PLAYERS 6
    ...
    typedef struct _CAPS
    {
        ...
        DWORD dwCallDelay; // the number of calls before data is sent over the network // between 2 and 8; single player
                           // is set to 1
    } CAPS, *PCAPS;
     ...
     *  Returns TRUE if the function was called successfully and FALSE otherwise.
     */
    BOOL STORMAPI SNetGetPlayerName(int playerid, char* buffer, unsigned int buffersize);
...
#define STRAFFIC_REPLY 4
    ...
    BOOL STORMAPI
    SNetJoinGame(int id, char* gameName, char* gamePassword, char* playerName, char* userStats, int* playerid);
    ...
    BOOL STORMAPI
    SNetReceiveTurns(int a1, int arraysize, char** arraydata, DWORD* arraydatabytes, DWORD* arrayplayerstatus);
...
#define SNPLAYER_OTHERS -2
     ...
     *  Returns TRUE if the function was called successfully and FALSE otherwise.
     */
    BOOL STORMAPI SNetSetGameMode(DWORD modeFlags, BOOLEAN makePublic);
...
#define SNET_GAME_RESULT_DISCONNECT 4
    ...
    BOOL STORMAPI
    SNetReportGameResult(unsigned a1, int size, int* results, const char* headerInfo, const char* detailInfo);
    ...
    HANDLE STORMAPI
    SDlgDialogBoxIndirectParam(HMODULE hModule, LPCSTR lpName, HWND hWndParent, LPVOID lpParam, LPARAM lParam);

    BOOL STORMAPI SDlgEndDialog(HWND hDlg, HANDLE nResult);

    BOOL STORMAPI
    SDlgSetControlBitmaps(HWND parentwindow, int* id, int a3, char* buffer2, char* buffer, int flags, int mask);
    ...
    */
    ...
    BOOL STORMAPI SDlgBltToWindowI(
        HWND hWnd,
        HRGN a2,
        char* a3,
        int a4,
        ...
        int a9,
        DWORD rop);
    BOOL STORMAPI SDlgBltToWindowE(
        HWND hWnd,
        HRGN a2,
        char* a3,
        int a4,
        ...
        DWORD rop);
    BOOL STORMAPI
    SDlgSetBitmapE(HWND hWnd, int a2, char* src, int mask1, int flags, int a6, int a7, int width, int a9, int mask2);
    ...
    BOOL STORMAPI
    SFileReadFile(HANDLE hFile, void* buffer, DWORD nNumberOfBytesToRead, DWORD* read, LONG* lpDistanceToMoveHigh);
    ...
    // 1: [TRUE|FALSE] - If true, reports resource leaks (SErrReportResourceLeak/SErrReportNamedResourceLeak) to the
    // attached debugger instead of a message box. 2: This option is unused. 3: [TRUE|FALSE] - If true, reports general
    // memory leaks to the attached debugger instead of a message box. 4: This option is unused. 5: [TRUE|FALSE] - If
    // true, reports log messages and log dumps to the attached debugger. 6: { DWORD blocks_allocated; DWORD
    // blocks_freed; } Used to determine the amount of memory/heap blocks that have been allocated and freed by storm.
...
#define SBMP_TGA 3
    ...
    HANDLE STORMAPI SBmpAllocLoadImage(
        const char* fileName,
    ...
    BOOL STORMAPI SDrawAutoInitialize(
        HINSTANCE hInst,
        LPCSTR lpClassName,
        LPCSTR lpWindowName,
        WNDPROC pfnWndProc,
     ...
     */
    BOOL STORMAPI SDrawCaptureScreen(const char* pszOutput);
    ...
    // undefined
    BOOL STORMAPI
    SDrawLockSurface(int surfacenumber, RECT* lpDestRect, void** lplpSurface, int* lpPitch, int arg_unused);
     ...
     *  Returns FALSE if no variables were specified.
     */
    BOOL STORMAPI SDrawManualInitialize(
        HWND hWnd,
        LPDIRECTDRAW ddInterface,
        LPDIRECTDRAWSURFACE primarySurface,
        LPDIRECTDRAWSURFACE surface2,
    ...
    BOOL STORMAPI
    SDrawUpdatePalette(unsigned int firstentry, unsigned int numentries, PALETTEENTRY* pPalEntries, int a4);
     ...
     */
    void* STORMAPI SMemAlloc(unsigned int amount, const char* logfilename, int logline, int defaultValue);
...
#define SMFree(loc) SMemFree((loc), __FILE__, __LINE__)
     ...
     */
    void* STORMAPI
    SMemReAlloc(void* location, unsigned int amount, const char* logfilename, int logline, char defaultValue);
...
#define SLOG_EXCEPTION -5
    ...
    BOOL STORMAPI
    SRegLoadData(const char* keyname, const char* valuename, int size, LPBYTE lpData, BYTE flags, LPDWORD lpcbData);
    BOOL STORMAPI
    SRegLoadString(const char* keyname, const char* valuename, BYTE flags, char* buffer, unsigned int buffersize);
...
#define SREG_ABSOLUTE 0x00000010              // specifies that the key is not a relative key
    ...
    BOOL STORMAPI
    STransCombineMasks(HANDLE hTransA, HANDLE hTransB, int left, int top, int flags, HANDLE* phTransResult);

    BOOL STORMAPI
    STransCreateE(void* pBuffer, int width, int height, int bpp, int a5, int bufferSize, HANDLE* phTransOut);
    BOOL STORMAPI
    STransCreateI(void* pBuffer, int width, int height, int bpp, int a5, int bufferSize, HANDLE* phTransOut);
     ...
     */
    BOOL STORMAPI SErrDisplayError(
        DWORD dwErrMsg,
...
#define SAssert(x)                                                                                                     \
    {                                                                                                                  \
        if (!(x))                                                                                                      \
            SErrDisplayError(STORM_ERROR_ASSERTION, __FILE__, __LINE__, #x)                                            \
    }
...
#define SEGetErrorStr(e, b) SErrGetErrorStr(e, b, sizeof(b))
    ...
    // groupID is a group in a MessageTable entry for example in STORM_ERROR_BAD_ARGUMENT 0x85100065, 0x510 is the
    // group. BOOL STORMAPI SErrRegisterMessageSource(WORD groupID, HMODULE hSourceModule, int a3)
...
#define SMFill(l, f) (SMemFill(l, sizeof(l), f))
     ...
     */
    void STORMAPI SMemZero(void* location, unsigned int length);
...
#define STORM_HASH_ABSOLUTE 1
    ...
    DWORD
    STORMAPI
    SStrHash(const char* string, DWORD flags, DWORD Seed);
...
#define SSCmpI(s, x) (SStrCmpI(s, x, __STORM_SSIZEMIN(s, x)))
     ...
     *  Returns TRUE if the user chose to continue execution, FALSE otherwise.
     */
    BOOL __cdecl SErrDisplayErrorFmt(
        DWORD dwErrMsg,
        const char* logfilename,
        int logline,
        BOOL allowOption,
     ...
     */
    unsigned int __cdecl SStrVPrintf(char* dest, unsigned int size, const char* format, ...);
    ...
    int __stdcall SNetInitializeProvider(
        unsigned long,

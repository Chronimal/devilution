...
const char* const szPlrModeAssert[12] = {"standing", "walking (1)", "walking (2)", "walking (3)",     "attacking (melee)", "attacking (ranged)",
...
static void DrawObject(int x, int y, int ox, int oy, BOOL pre, int CelSkip, int CelCap)
{
    ...
    else
    {
        ...
        if (object[bv]._oAnimData)
        { // BUGFIX: _oAnimData was already checked, this is redundant
...
static void DoBlitScreen(DWORD dwX, DWORD dwY, DWORD dwWdt, DWORD dwHgt)
{
    ...
    if (lpDDSBackBuf != NULL)
    {
        ...
        if (hDDVal != DDERR_SURFACELOST && hDDVal != DDERR_WASSTILLDRAWING && hDDVal != DDERR_SURFACEBUSY && hDDVal != DD_OK)
        {
